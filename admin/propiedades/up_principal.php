...
imagepng($nueva,$copia);
  ...
  header('location:../extend/alert.php?msj=Solo se acepta formato JPG y PNG&c=prop&p=img&t=error&id='.$id.'');
  exit;
}
...
if($up->execute()){
  ...
  header('location:../extend/alert.php?msj=Foto principal actualizada&c=prop&p=img&t=success&id='.$id.'');
}else{
  header('location:../extend/alert.php?msj=Foto principal no actualizada&c=prop&p=img&t=error&id='.$id.'');
}
...
header('location:../extend/alert.php?msj=Utiliza el formulario&c=prop&p=img&t=error&id='.$id.'');
